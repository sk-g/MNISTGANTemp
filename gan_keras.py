...
def generator(data):
    ...
    nch = 256
    model.add(Dense(nch*28*28,kernel_initializer='glorot_normal',activation = 'relu',input_shape = (100,)))
    model.add(BatchNormalization())
    model.add(keras.layers.core.Reshape( [nch, 28, 28] ))
...
def discriminator(data):
    ...
    shp = X_train.shape[1:]
    dropout_rate = 0.25
    dopt = keras.optimizers.Adam(lr=0.0009,
        decay = 1e-2)
    # Build Discriminative model ...
    #d_input = keras.layers.Input(shape=shp)
    discriminator = Sequential(name = "Discriminator_Network")
    ...
    discriminator.add(Conv2D(256, (3, 3),
        data_format="channels_first",
        strides=(2, 2), 
        padding="same"))
    discriminator.add(keras.layers.LeakyReLU(0.2))
    ...
    discriminator.add(Conv2D(512, (1, 1),
        data_format="channels_first", 
        padding="same"))
    discriminator.add(keras.layers.LeakyReLU(0.2))
    discriminator.add(Dropout(dropout_rate))    
...
def GAN(data,d,g):
    ...
    gan = Sequential(name = "GAN_Network")
    gan.add(g)
    gan.add(d)
    gan.compile(loss = 'categorical_crossentropy',optimizer = opt)
    graph = plot_model(gan, show_shapes = True, to_file = "gan.png")
...
def train_for_n(data,nb_epoch=5000, plt_frq=25,BATCH_SIZE=32):
    ...
    d.fit(X,y, verbose = 2,nb_epoch = 2, batch_size = 128)
    y_hat = d.predict(X)
    ...
    for e in tqdm.tqdm(range(1, nb_epoch+1)):
        # Make generative images
        image_batch = X_train[np.random.randint(0,X_train.shape[0],size=BATCH_SIZE),:,:,:]    
        noise_gen = np.random.uniform(0,1,size=[BATCH_SIZE,100])*0.001
        generated_images = g.predict(noise_gen)
...
data = load_data()
#d = discriminator(data)
#g = generator(data)
losses = train_for_n(data, nb_epoch = 25000, plt_frq = 500, BATCH_SIZE = 256)
plot_loss(losses)